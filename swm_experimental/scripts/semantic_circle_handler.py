...
class SemanticRegionHandler(object):
    def __init__(self):
        ...
        self._service = {}
        self._service['add_semantic_region'] = rospy.Service('add_semantic_region',AddSemanticRegion,self.processAddSemanticRegion)
        self._service['get_semantic_region'] = rospy.Service('get_semantic_region',GetSemanticRegion,self.processGetSemanticRegion)
        self._service['remove_semantic_region'] = rospy.Service('remove_semantic_region',RemoveSemanticRegion,self.processRemoveSemanticRegion)

    def processAddSemanticRegion(self,req):
        result = self.addSemanticRegion(req.name,req.pose_stamped.header,req.pose_stamped.pose)
        return AddSemanticRegionResponse("Region Added")

    def processRemoveSemanticRegion(self,req):
    ...
    def addSemanticRegion(self,name,header,Region):
    ...
    def processGetSemanticRegion(self,req):
        self._action['woits'].send_goal_and_wait(WorldObjectInstanceTagSearchGoal(self.tags))
        resp = self._action['woits'].get_result()
        ...
        return GetSemanticRegionResponse(p_stamped)
